...
public class OCVWebCamera extends Component {
    ...
    private void readPipline(Telemetry telemetry) {
        for (int i = 0; i < intoTheDeep_pipeline.means.length; i++) {
            telemetry.addData("rect " + i + " SAT", (int)intoTheDeep_pipeline.means[0].val[0]+" "
                    +(int)intoTheDeep_pipeline.means[i].val[1]+" "
                    +(int)intoTheDeep_pipeline.means[i].val[2]+" "
                    +(int)intoTheDeep_pipeline.means[i].val[3]);
        }
    }
    ...
    class IntoTheDeepPipline extends OpenCvPipeline {
        ...
        public Mat processFrame(Mat input) {
            ...
            for (int i = 0; i < means.length; i++) {
                maximum_index = (means[i].val[0] + means[i].val[2] - means[i].val[1])
                        > (means[maximum_index].val[0] + means[maximum_index].val[2] - means[maximum_index].val[1]) ? i : maximum_index;
            }
        ...
        private int [][] createRectangleArr(Mat input, int[][] rect_array) {
            double rect_base_offset = OCVWebCameraConfig.rects_offset_x;
            double rect_separation = rect_base_offset;
            rect_separation *= 2;
            rect_separation /= rect_array.length;

            if (rect_separation >= OCVWebCameraConfig.rect_separation) {
                rect_separation = OCVWebCameraConfig.rect_separation;
            }

            for (int i = 0; i < rect_array.length; i++) {
                rect_array[i] = new int[]{
                        (int) (input.cols() * (rect_base_offset + (rect_separation * i) - OCVWebCameraConfig.rect_size / 2)),
